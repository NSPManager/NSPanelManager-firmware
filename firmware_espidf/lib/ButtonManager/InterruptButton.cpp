// Original source: https://github.com/rwmingis/InterruptButton/blob/main/InterruptButton.cpp v2.0.2
...
//--------------------------------------------------------------------------------------------------------
uint32_t InterruptButton::m_RTOSservicerStackDepth{4096};
...
void IRAM_ATTR InterruptButton::readButton(void *arg) {
  ...
  case Released:                   // Was sitting released but just detected a signal from the button
    ...
    btn->m_validPolls = 1;
    btn->m_totalPolls = 1; // Was released, just detected a change, must be a valid press so count it.
  ...
  case Pressed:                                                                         // Currently pressed until now, but there was a change on the pin
    ...
    btn->m_validPolls = 1;
    btn->m_totalPolls = 1; // This is first poll and it was just released by definition of state
...
InterruptButton::InterruptButton(uint8_t pin, uint8_t pressedState, gpio_mode_t pinMode,
                                 ...
                                 uint16_t doubleClickMS, uint32_t debounceUS) : m_pressedState(pressedState),
                                                                                m_pinMode(pinMode),
...
void InterruptButton::initialiseInstance(void) {
  ...
  if (!m_classInitialised) { // We must be initialising the first button
    if (m_numMenus == 0)
      m_numMenus = 1; // Default to a single menu level if not set prior to initialising first button
